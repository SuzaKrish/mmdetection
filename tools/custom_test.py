...
def parse_args():
    ...
    parser.add_argument(
        '--use_cache',
        action='store_true',
        help='use cache of outputs saved in pickle file')
    parser.add_argument(
        '--format_only',
        action='store_true',
...
def main():
    ...
    if args.out is None:
        args.out = osp.join(cfg.work_dir,
                            '{}_{}'.format(timestamp, osp.basename(args.checkpoint).replace('pth', 'pkl')))
    ...
    if rank == 0:
        ...
        if args.format_only:
            save_dir = osp.join(cfg.work_dir, osp.basename(args.out).split('.')[0] + '_out')
